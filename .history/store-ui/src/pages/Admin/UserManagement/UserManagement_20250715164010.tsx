...
const UserManagement: React.FC = () => {
  ...
  const fetchUserOrders = async (userId: number) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/admin/users/${userId}/orders`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleUserAction = async (userId: number, action: 'block' | 'unblock' | 'suspend', reason?: string) => {
    ...
    try {
      ...
      switch (action) {
        case 'block':
          endpoint = `http://localhost:8080/api/admin/users/${userId}/block`;
          body = { reason };
          break;
        case 'unblock':
          endpoint = `http://localhost:8080/api/admin/users/${userId}/unblock`;
          break;
        case 'suspend':
          endpoint = `http://localhost:8080/api/admin/users/${userId}/suspend`;
          body = { reason };
          break;
      }

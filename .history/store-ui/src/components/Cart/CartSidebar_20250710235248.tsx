// src/components/Cart/CartSidebar.tsx (NEW FILE)
  ...
  const updateQuantity = async (productId: number, newQuantity: number) => {
    ...
    try {
      if (newQuantity <= 0) {
        ...
        // Update quantity
        await cartApi.updateCartItem(user.id, productId, newQuantity);
      }
        ...
        <Box sx={{ flex: 1, overflow: 'auto' }}>
          {loading ? (
            ...
            <Box sx={{ p: 2 }}>
              ...
              <Button onClick={fetchCart} variant="outlined" size="small">
                Retry
              </Button>
            </Box>
          ) : !cart || cart.items.length === 0 ? (

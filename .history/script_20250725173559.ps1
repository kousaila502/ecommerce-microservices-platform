...
$ScheduledTimes = @(
     # 5:30 PM - Run #3 (14 minutes from now)
    "18:15",  # 6:15 PM - Run #4  
    "19:00",  # 7:00 PM - Run #5
    "19:45",  # 7:45 PM - Run #6
    "20:30",  # 8:30 PM - Run #7
...
function Execute-GitOperations {
    ...
    try {
        ...
        # Add all changes (ignore CRLF warnings)
        Write-Log "Adding files to git..."
        $AddResult = & git add . 2>&1
        # Only fail on actual errors (exit code != 0), ignore warnings
        if ($LASTEXITCODE -ne 0) {
            throw "Git add failed with exit code $LASTEXITCODE`: $AddResult"
        }
        Write-Log "Files added successfully (warnings ignored)" "SUCCESS"
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Git commit failed with exit code $LASTEXITCODE`: $CommitResult"
        }
        Write-Log "Commit completed successfully" "SUCCESS"
        
        # Push to trigger pipeline
        Write-Log "Pushing to remote repository..."
        $PushResult = & git push 2>&1
        if ($LASTEXITCODE -ne 0) {
            throw "Git push failed with exit code $LASTEXITCODE`: $PushResult"
        }
        Write-Log "Push completed successfully" "SUCCESS"
        
        Write-Log "All Git operations completed successfully" "SUCCESS"
        Write-Log "Pipeline should be triggered automatically" "INFO"
        
    } catch {
        Write-Log "Git operations failed: $($_.Exception.Message)" "ERROR"
        # Log the actual git output for debugging
        Write-Log "Git add output: $AddResult" "DEBUG"
        Write-Log "Git commit output: $CommitResult" "DEBUG" 
        Write-Log "Git push output: $PushResult" "DEBUG"
        throw
    }
}

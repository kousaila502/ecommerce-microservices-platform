...
from fastapi import APIRouter, Depends, HTTPException, status, Header  # ADD Header import
...
async def create_order(
    ...
    print("🚀 create_order called")  # START trace

    order_service = OrderService(db)
    
    # Extract token
    token = authorization.replace("Bearer ", "") if authorization.startswith("Bearer ") else authorization
    print(f"🔐 Token extracted: {token[:10]}...")  # Log part of token
    
    try:
        print("📦 Calling order_service.create_order")

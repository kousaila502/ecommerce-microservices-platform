...
class OrderService:
    ...
    async def create_order(self, order_data: OrderCreate, user: User) -> Order:
        """Create a new order from cart items"""
        # 1. Get cart items
        cart_items = await self.get_cart_items(user.id)
        if not cart_items:
        ...
        for cart_item in cart_items:
            product = await self.get_product_details(cart_item['product_id'])
            if not product:
